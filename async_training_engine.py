...
"""
Async Training Engine for Nebula (Enhanced & Fixed)
...
# Optional TensorBoard: avoid import-time TensorFlow cascade
try:
    SummaryWriter = None  # Will be imported lazily only if needed
except Exception:
    SummaryWriter = None  # type: ignore

HAVE_TORCH = True

logger = logging.getLogger("Nebula.AsyncTraining")

# Use Pydantic v2 constraints
def Gt(x):
    return confloat(gt=x)
        ...
        for path_field in ["output_dir", "checkpoint_dir"]:
            ...
            if path_val:
                try:
                    ...
                    if not os.access(path_val, os.W_OK):
                        raise ValueError(f"{path_field} not writable: {path_val}")
                except OSError as e:
                    raise ValueError(f"Cannot create {path_field} at {path_val}: {e}")
...
class AsyncTrainingEngine:
    ...
    def __init__(
        self, vanta_echo_nebula_core: Any, default_engine_config: Optional[TrainingConfig] = None
    ):
        ...
        if hasattr(self.vanta_echo_nebula_core, "register_component"):
            ...
            logger.warning(
                "NebulaCore does not have register_component method. Skipping registration."
            )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self.vanta_echo_nebula_core and hasattr(self.vanta_echo_nebula_core, "publish_event"):
    ...
    async def _run_training_job(self, job: TrainingJob):
        ...
        try:
            ...
            if HAVE_TORCH and job.config.tensorboard_log_dir:
                try:
            ...
            if self.training_task and not self.training_task.done():
                try:
                    ...
                    try:
                        ...
                        logger.info("Training task forcibly cancelled")
                except asyncio.CancelledError:
                    logger.info("Training task cancelled during shutdown")
