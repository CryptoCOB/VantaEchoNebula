"""
Async Speech-to-Text (STT) Engine for Nebula
Uses Vosk for offline speech recognition with async support
Enhanced with features from CRYSTAL_HARMONY SWARM

DEPRECATED: All async STT (speech-to-text) requests should be routed through NebulaCore's async bus.
Do not instantiate or use this engine directly. Use NebulaCore and its async bus for all orchestration.

# Example (for maintainers):
# from vanta_echo_nebula_bus import VantaEchoNebulaCore
# vanta_echo_nebula_core = NebulaCore()
# vanta_echo_nebula_core.async_bus.register_component('stt_engine')
# vanta_echo_nebula_core.async_bus.subscribe('stt_engine', MessageType.AUDIO_TRANSCRIPTION, stt_engine.handle_audio_request)
"""
...
logger = logging.getLogger("Nebula.AsyncSTT")
...
class AsyncSTTEngine:
    ...
    def __init__(self, vanta_echo_nebula_core: Any, config: STTConfig):
        ...
        if hasattr(self.vanta_echo_nebula_core, "async_bus"):
            self.vanta_echo_nebula_core.async_bus.register_component("stt_engine")
    ...
    async def handle_audio_request(self, message):
        ...
        Use this method for all async STT requests via NebulaCore's async bus.
    ...
    async def _download_model_if_needed(self, loop: asyncio.AbstractEventLoop) -> bool:
        ...
        if not self.config.model_path and self.config.language:
            ...
            # Simplified model name for download URL. Actual Vosk models are archives like
            # vosk-model-small-en-us-0.15.zip. This should be updated to handle real
            # archive structures.
            model_archive_name = (
            ...
            try:
                ...
                if (
                    ...
                    # If extraction fails to produce the expected directory, raise an error.
                    # This part depends on the real Vosk model archive structures.
                    logger.warning(
...
# --- NebulaCore mock for example usage if not imported ---
class NebulaCore:
...
# Example Usage (requires a mock vanta_echo_nebula_core and proper model setup)
# async def main():
#     logging.basicConfig(
#         level=logging.INFO,
...
#     stt_engine = AsyncSTTEngine(vanta_echo_nebula_core=NebulaCore(), config=config)
