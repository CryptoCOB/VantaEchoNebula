"""
DEPRECATED: All async TTS (text-to-speech) requests should be routed through NebulaCore's async bus.
Do not instantiate or use this engine directly. Use NebulaCore and its async bus for all orchestration.
"""

# Async Text-to-Speech (TTS) Engine for Nebula
...
from vanta_echo_nebula_bus import MessageType
...
logger = logging.getLogger("Nebula.AsyncTTS")
...
class AsyncTTSEngine:
    """Async Text-to-Speech Engine for Nebula."""

    COMPONENT_NAME = "async_tts_engine"

    def __init__(self, vanta_echo_nebula_core, config: Optional[TTSConfig] = None):
        ...
        if hasattr(self.vanta_echo_nebula_core, "async_bus"):
            self.vanta_echo_nebula_core.async_bus.register_component("tts_engine")
        ...
        with self._cache_lock:
            if len(self._audio_cache) >= self.config.cache_max_size:
                ...
                try:
                    ...
                    logger.warning(f"Failed to remove cached file {old_path}: {e}")
    ...
    def _publish_result(self, result: SynthesisResult):
        """Publish synthesis result to the Nebula ecosystem."""
        try:
    ...
    async def handle_speech_request(self, message):
        ...
        Use this method for all async TTS requests via NebulaCore's async bus.
        Args:
    ...
    def cleanup(self):
        ...
        with self._cache_lock:
            for audio_path in self._audio_cache.values():
                try:
                    ...
                    logger.debug(f"Failed to delete cached audio {audio_path}: {e}")
        ...
        if "pyttsx3" in self._engines and "engine" in self._engines["pyttsx3"]:
            try:
                ...
                logger.debug(f"Error stopping pyttsx3 engine: {e}")
...
def create_async_tts_engine(
    vanta_echo_nebula_core, config: Optional[TTSConfig] = None
) -> AsyncTTSEngine:
    """Create and return an AsyncTTSEngine instance."""
    return AsyncTTSEngine(vanta_echo_nebula_core, config)
