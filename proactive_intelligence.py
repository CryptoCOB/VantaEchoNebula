...
"""
Proactive intelligence and priority management for the NebulaCore ecosystem.
...
# Assuming vanta_echo_nebula_core.py is accessible
from vanta_echo_nebula_bus import VantaEchoNebulaCore
# Setup logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
)
logger = logging.getLogger("NebulaCore.ProactiveIntelligence")
...
class TaskSchedulerInterface(
    Protocol
):  # Interface for task_scheduler component from VantaEchoNebulaCore
...
class ProactiveIntelligence:
    ...
    def __init__(
        ...
        if not isinstance(model_manager, ModelManagerInterface):
            # Try to get from VantaEchoNebulaCore registry if not provided or wrong type
            mm_from_registry = self.vanta_echo_nebula_core.get_component("model_manager")
            if isinstance(mm_from_registry, ModelManagerInterface):
                self.model_manager = mm_from_registry
                logger.info("Using 'model_manager' from VantaEchoNebulaCore registry.")
            else:
                # This is a critical dependency, should ideally raise error or have a more robust default.
                # For now, log a warning since we have a functional fallback. NebulaCore setup should ensure it.
                logger.warning(
                    "ModelManagerInterface not provided or found in NebulaCore registry. Using DefaultStubModelManager fallback."
                )
        ...
        logger.info("ProactiveIntelligence initialized and registered with NebulaCore.")
    ...
    def handle_action_evaluation(self, message: AsyncMessage) -> None:
        ...
        asyncio.create_task(
            self.vanta_echo_nebula_core.async_bus.publish(
                AsyncMessage(
    ...
    def _get_task_scheduler(self) -> TaskSchedulerInterface | None:
        scheduler = self.vanta_echo_nebula_core.get_component(
    ...
    def _get_metrics_store(self) -> MetricsStoreInterface | None:
        store = self.vanta_echo_nebula_core.get_component(self.config.metrics_store_component_name)
    ...
    def _get_health_monitor(self) -> HealthMonitorInterface | None:
        monitor = self.vanta_echo_nebula_core.get_component(
    ...
    def evaluate_action(
        ...
        self._record_action_evaluation(
            evaluation
        )  # Changed name for clarity        # Publish evaluation event to NebulaCore
    ...
    def _evaluate_system_command(
        ...
        if required_deps:
            missing_deps = [
                ...
                if self.vanta_echo_nebula_core.get_component(dep) is None
            ]
            if missing_deps:
                eval_results["risk_score"] = max(
    ...
    def _apply_event_impact_to_prediction(self, pred: dict, event: dict):
        ...
        for k, delta in effects.items():
            pred[k] = pred.get(k, 0.0) + delta
        risk = event.get("risk_delta")
        if risk is not None:
            pred["risk_score"] = max(0.0, pred.get("risk_score", 0.0) + risk)

    def _analyze_system_trends(self) -> list[dict[str, Any]]:
        counts = Counter(ev.get("action_type") for ev in self.action_history)
        return [{"action_type": act, "frequency": freq} for act, freq in counts.items()]

    def _apply_trend_impact_to_prediction(self, pred: dict, trend: dict, win_s: int):
        weight = trend.get("weight", 0.0)
        for k in pred:
            pred[k] += weight * win_s
...
if __name__ == "__main__":
    ...
    vanta_system_pi = NebulaCore()

    # 2. Mock ModelManager (would be a real component provided by NebulaCore or injected)
    ...
    proactive_intel_instance = ProactiveIntelligence(
        vanta_echo_nebula_core=vanta_system_pi,
    ...
    # Mock some other components ProactiveIntelligence might query via vanta_echo_nebula_core.get_component()
    ...
    main_logger_pi.info("\n--- Proactive Intelligence NebulaCore Example Finished ---")
